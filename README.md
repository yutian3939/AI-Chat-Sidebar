...
- 🎨 **MD3 主题** — Material Design 3，6 套配色方案 × 亮色/暗色/跟随系统
...
### 🖼️ 图片理解 & 文件上下文
- 📎 **附件上传** — 输入框上方 `+` 按钮，支持图片（缩略图预览）和文本文件
...
> 修改代码后需在 `chrome://extensions/` 点击扩展卡片的 🔄 刷新，**再刷新目标页面**。
...
| 方案 | 说明 |
|------|------|
| **使用主模型** | 图片以 `image_url` 多模态格式直发（需 gpt-4o / qwen3.X-plus 等视觉模型） |
...
├── background.js           # Service Worker：流式/非流式聊天、多模态、视觉转述、自动答题 API
...
│   ├── settings-panel.js   # 设置面板（供应商/主模型/视觉模型/主题管理）
